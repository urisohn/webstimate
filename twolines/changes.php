...
<font color='gray' size='4'>
	v0.52 - 2018 11 23<BR>
	<ul>
		<li>Modified how fitted values are computed for plotting the scatterplot smoother. In particular, rather than
		fitting at mean values of covariates, as done until now, data are fitted for the "median" value of the covariate; in the case of a factor (i.e., categorical) variable, 
		it is the value of the factor at the n/2 observation, after sorting. For example, if z=c("doctor", "nurse", "artist") the fitted value would be for z="doctor", the middle observation
		once the factor is sorted.  This change follows the observation by Conor Wild from U of Waterloo that the app would give an error message when used with factor variables.</li></ul>
	<BR>
	
	v0.51 - 2018 08 27<BR>
	<ul>
		<li>Fixed bug where missing values for one of the variables would cause the program to crash.</li></ul>
	<BR>

	
	v0.5 - 2018 06 26<BR>
	<ul>
		<li>Modified template figure, it now shows results for two-lines as arrows with average slopes instead of as fitted regressions lines.
		This seeks to avoid confusion about what the two-lines test does. It does compute average slope, it does not seek to model the data.</li></ul>
	<BR>
	...
	<ul>
		...
		Thanks to Joseph Reiff (UCLA PhD student) for identifying the bug and the line that was generating it</ul>
	<BR>
	v0.32 - 2018 03 20<BR>
	<ul>
	...
	<li>It was realized that using HC3 to estimate heteroskedasticity robust standard errors  creates NA 
		values under some circumstances leading the app to crash. For a few hours the app was switched to relying on HC1 instead HC3. But see App 0.32.<BR>
