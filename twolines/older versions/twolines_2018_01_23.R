...
#Function 3 - reg2() two-line regression with glm()
  ...
  #Function 4- 
      twolines=function(f,graph=1,link="gaussian",data=NULL,pngfile="")  {
        if (!is.null(data)) attach(data)         #If data is specified, then use attach() command to make all variables available without needing to specify the dataset
        ...
        #2 Run smoother 
              ...
              #2.2 Now run it
                gams=gam(as.formula(gam.f),link=link)  #so this is a general additive model with the main specification entered
                                                       #but we make the first predcitor, the one that will be tested for having a u-shaped effect
                                                       #be estimated with a completely flexible functional form.
                                                       #gam.f looks like this:
                                                       #  y~s(x1)+x2+x3
