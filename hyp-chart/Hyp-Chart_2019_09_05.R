   ...
   #Function 2 - Format likelihood for presentation in chart
     ...
     #Function 3 - Simplify notation and output of how Bayes Factor is calculated 
         bf=function(n,d) {
              t=(d/2)*sqrt(2*n)
              #cat("d=",d,"-->=",t,"\n")
              exp(ttest.tstat(t,n1=n,n2=n)$bf)
           }

    #Function 4 - The HypChart function
    hyp.chart=function(d.obs=NA,n,ylim=NA,p.obs=NA,
      ...
      #If p.obs is set, and not d.obs, compute t.obs and d.obs
        if (!is.na(p.obs)) 
            {
      ...
      #If d.obs is set, compute p.obs and t.obs 
         if (!is.na(d.obs))
            {
    ...
    #Get likelihoods
        ...
        l.all=dt(t.obs,df=df,ncp=sqrt(n/2)*d) #likelihood for all hypotheses
    ...
    #ylim:  if ylim not assigned, set it to observed rate, plus a buffer
      if (is.na(ylim)) ylim=c(min(r.all)*.9,max(r.all,5)*1.25)
    ...
    #Plot
        ...
        #Add Bayes results on the left of figure?
           if (show.bayes==TRUE) {
            ...
            #Text with results
                bf.obs=bf(n=n,d=d.obs)
                text(-1.85,bf.obs,pos=3,offset=.1,paste0("Bayes Factor = ",round(bf.obs,1)),col='red4',font=2)
    ...
    #Horizontal line at 1
       ...
       #X-axis
         #Ticks
          ...
          #Tick at d.obs
            ...
            text(d.obs,0,bquote(hat(d)*" = "*.(round(d.obs,2))),cex=.75,col='red4')
      ...
      #DOTS - t-shirts
         ...
         #RED DOT - observed value
          l.obs=dt(t.obs,df,ncp=sqrt(n/2)*d.obs)
          r.obs=l.obs/l.0
          points(d.obs,r.obs,col='red4',pch=23,cex=1.25,lwd=2)
    ...
    #Legend
         ...
         leg=legend("topleft", inset=.011, pch=c(16,1,23),
              col=c("black","blue2","red4"),box.col='white',
              pt.cex=c(1,2,1.5), lwd=c(1,2,2), lty=NA,
              legend=c(leg.dot,
