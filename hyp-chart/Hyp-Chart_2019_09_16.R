  ...
  #Function 1- Format p-values nicely 
   ...
   #1.2 If ONE samples
    ...
    #ylim:  if ylim not assigned, set it to observed rate, plus a buffer
      if (is.na(ylim[1])) ylim=c(min(r.all)*.9,max(r.all,5)*1.25)
    ...
    #Plot
        ...
        #Add Bayes results on the left of figure?
           if (show.bayes==TRUE) {
            ...
            #Text with results
                bf.obs=bf(n=n,d=d.obs,one.sample = one.sample)
         ...
         #Ticks
          ...
          #Show CI in the x-axis
              if (show.CI==T)
                ...
                #segments
                   text(d.obs,.085*length(ylim),bquote(hat(d)*" = "*.(round(d.obs,2))),cex=.75,col='red4')
                   segments(x0=ci[1],x1=ci[2],y0=.0,y1=0,col='red4',lwd=1.75)      #Horizontal line
                   segments(x0=ci[1],x1=ci[1],y0=-.05,y1=.05,col='red4',lwd=1.75)  #Cap 1
                   segments(x0=ci[2],x1=ci[2],y0=-.05,y1=.05,col='red4',lwd=1.75)  #Cap 2
    ...
    #Legend
        ...
        #Including CI? If not, print it in white so it is not visible
            ...
            if (show.CI==T) {
                ...
                leg=legend("topleft", inset=.011, pch=c(16,1,23,NA),
                    lty=c(NA,NA,NA,1),
                    col=c("black","blue2","red4",col.CI),box.col='white',
                   pt.cex=c(1,2,1.5,NA), lwd=c(1,2,2,2), 
